...
class CameraDebugWindow(QWidget):
    def __init__(self, parent=None):
        ...
        # Internal State (Updated by logic signal)
        self.avg_ratio = None
        self.limit_ratio = None
        self.eyes_off = True

    def update_logic(self, avg_ratio, eyes_off, limit_ratio):
        """
        Slot to receive lightweight logic data.
        Handles float sentinels (-1.0) from the worker.
        """
        self.avg_ratio = avg_ratio if avg_ratio != -1.0 else None
        self.limit_ratio = limit_ratio if limit_ratio != -1.0 else None
        self.eyes_off = eyes_off

    def update_frame(self, cv_frame):
        """
        Slot to receive heavy video frame.
        Draws the UI using the stored internal state.
        """
        ...
        # Draw Limit Line
        if self.limit_ratio is not None:
            limit_y = int(bar_top + (self.limit_ratio * bar_height))
        ...
        if self.avg_ratio is not None:
            # Clamp ratio 0.0-1.0
            safe_ratio = max(0.0, min(1.0, self.avg_ratio))
            current_y = int(bar_top + (safe_ratio * bar_height))
            
            color = (0, 255, 255) # Yellow
            if self.eyes_off: color = (0, 0, 255)
